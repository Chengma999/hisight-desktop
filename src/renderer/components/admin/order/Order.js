...
import { Collapse,List ,Pagination,Button,Modal,Row,Col,Popover,Typography} from 'antd';
import {PrinterOutlined,MessageOutlined,InfoCircleOutlined } from '@ant-design/icons'
import styles from '../admin.less'
import {printOrder, smsSend} from '../../../actions/index'
...
class Order extends Component {
  ...
  render() {
    ...
    const {loading,admincrud,printOrder,smsSend} =this.props
    //get all paid orders from DB
    const allPaidOrders= loading.effects["admincrud/fetchOrders"]===false?admincrud.fetchOrders.orders:[]
    const limitedPaidOders = allPaidOrders.filter(order=>chosenDate.isSame(moment(order.createdDate),'day'))
    const allPanels=limitedPaidOders.map((order,i)=>{
        totalSalesRevenue+=order.totalPrice
       if( order.orderType==='afhalen') {totalAfhaalRevenue+=order.totalPrice}
       if( order.orderType==='bezorgen') {totalBezorgenRevenue+=order.totalPrice}
        const content=(
        <div>
        ...
        const header=(
          <Row>
            <Col span={6}><b>{order.cus_orderId}</b></Col>
            <Col span={6}>
              ...
              <b> {order.orderType} {order.takeTime}</b>
              </Col>
            <Col span={6}><b>{order.customerName}</b></Col>
            <Col span={6}><b>{convertInEuro(order.totalPrice)}</b></Col>
          </Row>)
      return(
          <Collapse.Panel header={header} key={i}>
            ...
            return <div className={styles.productsHeader}>
              <Row>
                ...
                <Col span={6}>{cartProduct.option!==undefined?cartProduct.option.title:'nvt'}</Col>
                <Col span={6}>{convertInEuro(cartProduct.subTotal)}</Col>
              </Row>
            </div>
            })}
            <div className={styles.printer}>
              <span style={{paddingRight:'20px'}}> <Button type="primary"  shape="circle" icon={<PrinterOutlined />} onClick={()=>printOrder(order)}></Button>
              </span>
              <span> <Button type="danger"  shape="circle" icon={<MessageOutlined />} onClick={()=>this.showModal(order)} ></Button>
              </span>
            </div>
          </Collapse.Panel>
        )})
  ...
  <Collapse bordered={true} defaultActiveKey={['0']}>
    {allPanels.filter((a,i)=>i>= (current-1)*pageSize&&i<=current*pageSize-1 )}
  </Collapse>
   ...
   <Modal
    ...
    visible={visible}
    title={order.cus_orderId +' '+order.orderType + ' '+ order.customerName+' '+convertInEuro(order.totalPrice) }
    onOk={this.handleOk}
...
export default connect (mapStateToProps,{printOrder,smsSend})(Order)
